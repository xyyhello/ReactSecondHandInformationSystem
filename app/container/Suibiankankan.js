...
import NavBlock from '../component/suibiankankan/NavBlock.js';
					...
					<nav className="mynav">
						...
						<div>
							{alltypes.map((item,key)=>{
								return <Link  key={key} activeClassName="cur" to={"suibiankankan/" + item.url}><NavBlock key={key} name={item.typename} classname={item.classname}></NavBlock></Link>
						})}
